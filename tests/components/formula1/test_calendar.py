...
pytestmark = pytest.mark.usefixtures("config_entry", "coordinator")


MOCK_DATETIME = dt_util.as_utc(datetime(2017, 11, 27, 0, 0, 0))


def test_create_calendar_event(hass: HomeAssistant, config_entry, coordinator):
    """Test creating a calendar event."""
    calendar = Formula1Calendar(coordinator, config_entry)
...
@pytest.mark.asyncio
async def test_async_get_events_race(hass: HomeAssistant, config_entry, coordinator):
...
async def test_async_get_events_no_events(
    hass: HomeAssistant, config_entry, coordinator
):
